-- Roll up one day into daily_summary. Idempotent via MERGE on `day`.
-- target defaults to yesterday; override by editing the DECLARE before running.
DECLARE target DATE DEFAULT DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY);
